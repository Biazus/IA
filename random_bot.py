...
class Player(LiacBot):
    ...
    def on_move(self, state):
        ...
        moves = board.generate()
        enemymoves = board.generateEnemy()
        print state
...
class Board(object):
    def __init__(self, state):
        ...
        for row in xrange(7, -1, -1):
            for col in xrange(0, 8):
                if c[j] != '.':
                    cls = PIECES[c[j].lower()]
                    team = BLACK if c[j].lower() == c[j] else WHITE
...
class Pawn(Piece):
    ...
    def generate(self):
        ...
        # Movement to 2 forward (first play)
        pos = (my_row + d*2, my_col)
        #print self.team
        if self.board.is_empty(pos) and ((my_row == 6 and self.team == BLACK) or (my_row == 1 and self.team == WHITE)):
            moves.append(pos)
